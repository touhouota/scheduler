let ProgressTimer = {
	...
	display: function(task_element) {
		// 表示を変更する部分
		let timer = task_element.querySelector(".real > .time");
		// タスクに付随している情報から、時間を計測する
		let seconds = ProgressTimer.calc_diff_seconds(task_element);
		// 表記を書き換える
		timer.textContent = ProgressTimer.convert_hms_from_seconds(seconds);
	},
